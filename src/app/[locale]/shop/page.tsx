    ...
    <div className="bg-[#f8f9fa] flex-1 flex flex-col">
      ...
      >
        ...
        <div className="relative z-10 max-w-[1440px] mx-auto px-4 lg:px-8 text-center flex flex-col items-center">
          <h1 className="text-4xl md:text-5xl font-extrabold text-brand-dark mb-4 tracking-tight">{t('title')}</h1>
        ...
        <section className="flex-1">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-8 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 gap-4">
            <p className="text-gray-500 font-medium text-sm">
              {t('foundPrefix')} <span className="font-bold text-brand-dark">{filteredProducts.length}</span> {t('foundSuffix')}
            </p>
          ...
          {filteredProducts.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProducts.map((product: any) => (
                <a key={product.id} href={`${lp}/product/${product.slug}`} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full border border-gray-100">
                  <div className="relative aspect-square bg-gray-50 p-8 flex items-center justify-center">
                    {product.images && product.images.length > 0 ? (
                      ...
                      <div className="text-gray-400 font-medium">{tc('noImage')}</div>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100 flex flex-col items-center justify-center">
              ...
              <h3 className="text-xl font-bold text-brand-dark mb-2">{t('noneTitle')}</h3>
              <p className="text-gray-500 mb-6">{t('noneDesc')}</p>
              <a href={`${lp}/shop`} className="bg-brand-primary text-white px-8 py-3 rounded-full font-bold hover:bg-opacity-90 transition-all">
                {t('backToAll')}
              </a>
            </div>
          )}
