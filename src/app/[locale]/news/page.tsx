...
import { getPostsData } from "@/lib/cms";

const getPosts = () => getPostsData();
      ...
      <main className="flex-1 max-w-[1440px] mx-auto px-4 lg:px-8 py-16 lg:py-24 w-full">
        {posts && posts.length > 0 ? (
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] bg-gray-50 overflow-hidden">
                    {featuredImage ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 bg-gray-100">
                        ...
                        <span className="text-sm font-medium">{t('noFeatured')}</span>
                      </div>
                    )}
                  ...
                  <div className="p-8 flex flex-col flex-1 relative">
                    ...
                    <div className="mt-auto flex items-center text-brand-primary font-bold text-sm">
                      {t('readMore')} <ArrowRight className="w-4 h-4 ml-1.5 transform group-hover:translate-x-1.5 transition-transform" />
                    </div>
                  </div>
                </a>
              );
          ...
          <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100 flex flex-col items-center justify-center min-h-[400px]">
            ...
            <h3 className="text-2xl font-extrabold text-brand-dark mb-3">{t('noPosts')}</h3>
            <p className="text-gray-500 font-medium">{t('noPostsDesc')}</p>
          </div>
        )}
      </main>
    </div>
