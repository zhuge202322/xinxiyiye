...
export default async function ArticleDetailPage({ params }: { params: Promise<{ slug: string }> | { slug: string } }) {
  ...
  const lp = `/${locale}`;
  const dateLocale = locale === 'ar' ? 'ar' : locale === 'fr' ? 'fr-FR' : locale === 'es' ? 'es-ES' : 'en-US';
    ...
    <div className="bg-[#f8f9fa] flex-1 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-100 py-6">
        <div className="max-w-[1440px] mx-auto px-4 lg:px-8">
          <p className="text-gray-500 font-medium flex items-center text-sm">
            ...
            <a href={`${lp}/news`} className="hover:text-brand-primary transition">{t('breadcrumb')}</a>
            <ChevronRight className="w-4 h-4 mx-2" /> 
      ...
      <main className="flex-1 max-w-[1000px] mx-auto px-4 lg:px-8 py-16 lg:py-24 w-full">
        ...
        <div className="mb-12 text-center">
          <a href={`${lp}/news`} className="inline-flex items-center text-brand-primary font-bold text-sm mb-8 hover:underline">
            <ArrowLeft className="w-4 h-4 mr-2" /> {t('backToBlog')}
          </a>
          <h1 
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-gray-500 font-medium text-sm border-y border-gray-200 py-6">
            ...
            <span className="flex items-center">
              <Clock className="w-4 h-4 mr-2 text-brand-primary" /> {t('minRead', { n: 5 })}
            </span>
            <button className="flex items-center hover:text-brand-primary transition-colors">
              <Share2 className="w-4 h-4 mr-2 text-brand-primary" /> {t('share')}
            </button>
          </div>
        </div>
        ...
        <article className="bg-white rounded-[3rem] shadow-sm border border-gray-100 p-8 md:p-16 lg:p-20">
          <div 
            ...
            "
            dangerouslySetInnerHTML={{ __html: content || `<p>${t('noPosts')}</p>` }}
          />
          
          <div className="mt-16 pt-8 border-t border-gray-100 flex justify-between items-center">
            <div className="flex gap-3">
              <span className="bg-brand-secondary text-brand-primary px-4 py-2 rounded-full text-xs font-bold">{t('tagIndustry')}</span>
              <span className="bg-brand-secondary text-brand-primary px-4 py-2 rounded-full text-xs font-bold">{t('tagNewProducts')}</span>
            </div>
          </div>
        </article>
      </main>
